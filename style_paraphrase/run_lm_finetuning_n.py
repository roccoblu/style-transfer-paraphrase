...
"""Fine-tuning GPTNeo for conditional generation tasks."""
...
from transformers import (WEIGHTS_NAME, AdamW, GPTNeoConfig, GPTNeoForCausalLM,
                          GPT2Tokenizer, get_linear_schedule_with_warmup)
from transformers.models import gpt_neo

from utils import GPTNeoParentModule, init_gpt_neo_model
...
def save_model(gpt_neo_model, output_dir, args, global_step, tokenizer=None):
    # Take care of distributed/parallel training
    model_to_save = gpt_neo_model.gpt_neo
...
def train(args, gpt_neo_model, train_dataset, tokenizer):
    """ Train the model """
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:
                        results = evaluate(args, gpt_neo_model, tokenizer)
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    save_model(gpt_neo_model, output_dir, args, global_step, tokenizer=tokenizer)
...
def evaluate(args, gpt_neo_model, tokenizer, prefix=""):
    ...
    # multi-gpu evaluate
    if args.n_gpu > 1:
        gpt_neo_model = torch.nn.DataParallel(gpt_neo_model)
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        curr_loss = gpt_neo_model.evaluate(batch)
        eval_loss += curr_loss
...
def main():
    ...
    gpt_neo_model = GPTNeoParentModule(args=args, gpt_neo=model)
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(args, gpt_neo_model, train_dataset, tokenizer)
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        if not os.path.exists(output_dir) and args.local_rank in [-1, 0]:
            os.makedirs(output_dir)
        save_model(gpt_neo_model, output_dir, args, global_step, tokenizer)

        gpt_neo_model, tokenizer = init_gpt_neo_model(checkpoint_dir=output_dir,
                                                args=args,
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        while not eval_done:
            ...
            for checkpoint in checkpoints:
                ...
                gpt_neo_model, _ = init_gpt_neo_model(checkpoint_dir=checkpoint,
                                                args=args,
                                                model_class=model_class)

                result = evaluate(args, gpt_neo_model, tokenizer, prefix=prefix)
